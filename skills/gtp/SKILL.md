...
8. Identify every Decision Record whose selected means the current task creates, changes, or implements. When the task includes a pull request body or commit message, add the one-way reference defined by `GTP.md` for every such record, even if the record itself did not change. Do not perform an external write unless the user authorized that operation.
9. Verify that each created or updated record contains the current selected means. For every changed means, verify that its change-history entry states what changed and includes a pull request, commit, or Issue link when one is available.
10. If the recorded decision no longer applies, keep the file and use the no-longer-applicable procedure defined by `GTP.md`.
11. Before handing off or externally writing a human-readable artifact, run the pre-submission review below for that artifact unless the user explicitly named it and requested that this review be skipped. Run the review whether or not the recording criteria produced a Decision Record.
