...
| 4 | どこを直すか | expected resultとobserved resultの比較Evidenceで責任を確定できた最小の修正対象。未確定なら「修正責任未確定」 | 関係のないrepository、protocol vocabulary |
...
3. observed resultがexpected resultと異なる場合は、入力のvalidityにかかわらず、そのobserved resultを返したproduction pathのowner implementationを修正責任として確定する。`gtp status`のreader／CLIまたは`gtp check`のproduction pathならGTP implementation、setup／adapter preflightならそのsetup workflow owner、外部Operation／providerならそのOperation／provider ownerとする。例えばExact Marker付きの壊れたJSONをGTP reader／CLIが期待される`halt / invalid_record`ではなくcrashまたは`unmanaged`として返す場合は、GTP implementationが修正責任を持つ。
...
5. expected result、observed result、またはobserved resultを返したproduction pathのownerを固定できず、比較Evidenceまたはowner Evidenceが不足する場合は、根本的な修正責任を「修正責任未確定」とする。
