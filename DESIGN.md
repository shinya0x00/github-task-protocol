...
- `v1.0.3.post1`公開時は、pull requestで`github.event.pull_request.head.sha`、`legacy/1.x` pushで`github.sha`を唯一の`SOURCE_SHA`とした。checkout、`SOURCE_DATE_EPOCH`導出、`git ls-tree`、`git archive`、build、artifact名はすべてこのSHAを使った。pull requestのmerge refはsource identityに使っていない。
...
- 公開時のPR artifactは検証専用の`v1.0.3.post1-pr-verification-<SOURCE_SHA>`とした。squash merge後の`legacy/1.x` push artifactだけを公開候補とし、`v1.0.3.post1-legacy-candidate-<SOURCE_SHA>`とした。Python 3.11、Python 3.12、Python 3.13のconsumerはproducerがuploadした同じartifactをdownloadし、checksum、clean install、installed CLI、unit testを検査した。
...
- workflowは`contents: read`だけを使い、tag、GitHub Release、PyPIを変更しない。公開時は`legacy/1.x` artifactのsource SHA、run、artifact ID、expiry、build条件、filename、size、SHA-256、再検査手順をoperation recordへ引き継いだ。
- `1.0.3.post1`の公開とPyPI archive後、current workflowはpull requestだけを検証する。`legacy/1.x` pushで新しい公開候補を作らない。公開後Evidenceは`legacy/1.x`のrepository-only fileとしてversion管理し、sdistとwheelに含めない。このEvidence PRのmerge後は`legacy/1.x`を運用上凍結し、追加変更を行わない。source identityはannotated tag `v1.0.3.post1`がdereferenceするexact commitである。
