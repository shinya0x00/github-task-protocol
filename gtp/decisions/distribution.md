...
標準インストールには外部CLI `skills`を使う。Release asset `github-task-protocol-v2.0.9.tar.gz`を`SHA256SUMS`で検証し、CLI `skills` v1.5.21のtarballもSHA-512で検証してから、ローカルの`skills/`を二つのSkill名、対象Agent、global scope、copy方式とともに一回の実行へ渡す。二つを同じReleaseからAgentのuser-level認識先へ別directoryとして配置し、インストール後に片方が不要なら、そのSkill directoryだけを削除できる。標準install setは、二つのSkillの実行時の独立性を変えない。

rootの`GTP.md`は既存参照向けのprojectionとし、導入時のcopy対象に含めない。Agent別の`SKILL.md`、GTP独自のpackage、複数Skill用manifest、installer、install stateは作らない。外部CLIのlockまたはprovenance情報はCLIの所有物とし、二つのSkillはruntimeで参照しない。各`agents/openai.yaml`はOpenAI向けの任意metadataとして扱う。PythonとPyPIを標準配布経路にしない。Node.jsまたは`npx`を使わない場合も、`SHA256SUMS`で検証した同じRelease source archiveから二つを手動配置できる。
...
- v2.0.9で、mutableなCLI版とtag名だけに依存する導入をやめ、Release archiveのSHA-256と固定版CLI tarballのSHA-512を検証してから、検証済みローカルsourceへ外部CLIを実行する方式へ変更。
