...
Pre-submission Review Skillが、Decision Record、Issue、pull request、commit message、Release notes、利用者向け説明などの人間可読な文章を、どの言語で作るか。

## 採用した手段

Pre-submission Review専用のinstall時設定や永続する言語設定fileは追加しない。review対象となる人間可読な文章の言語は、対象ごとに次の順で決める。
...
Agentが従うべきinstructionの優先順位を先に守る。言語の指定も判断材料もないsourceは飛ばして次へ進む。言語を明示しないtemplateは言語を決める根拠にしない。Pre-submission Review Skill自身のinstruction、UI metadataや自動生成されたdefault prompt、例文は、利用者またはrepositoryの言語を決める根拠にしない。最初に判断材料がある優先順位の中で候補が一つに決まらない場合、または最後まで一つに決まらない場合だけ、その対象について利用者へ一度確認する。解決するまで文章を作成または外部投稿しない。
