...
3. `未接続dependency`: 変更せず、外部Operation接続のblockerを報告する。
...
preflightとblocker報告はephemeralです。working tree、branch、Issue、comment、label、PRを変更せず、repair Issueも自動作成しません。blocker解消後は同じ入力でpreflightを再実行します。`instructionなし`または`両立可能`の場合だけ、次の既存branch-first手順へ進みます。
