...
CLIは現在、PyPI、npm、その他のpackage registryへ一般公開していません。repository内で仕様の受け入れと検証に使います。将来の配布方法は未決定です。

```console
PYTHONPATH=src python3 -m gtp status <issue-url>
PYTHONPATH=src python3 -m gtp check <comment.md>
```
