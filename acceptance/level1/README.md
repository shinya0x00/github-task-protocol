...
repair Issue #34をmainへmergeし、新candidateでmachine probeとhuman probeを
最初から再実行した。machine側は成功し、人間は停止、次の行動、範囲、作業先、
`proof_b`不足を判断できた。

一方、何が証明済みかは判断できず、「見る場所は分かりやすくなったが、
非エンジニアにはまだ負荷が高く、噛み砕いた説明がない」と評価された。
Issue #38で平易な要約を修復し、新candidateで再実行するまでIssue #13のDone、
ready化、mergeを行わない。
...
| human probe | PR #33 response | 見る場所は改善したが、証明済み項目が不明で、平易な説明不足により再度受け入れ失敗 |
